...
class ChatWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Флаг использования долгосрочной памяти (клиент более не отвечает за RAG)
        # Оставляем False по умолчанию, чтобы избежать ошибок при обращении
        self.use_long_term_memory: bool = False
    ...
    def _check_crewai_availability(self):
        ...
        # Check embedded memory system (SimpleMemoryManager) - DISABLED, RAG is now server-side
        # try:
    ...
    def show_context_stats(self):
        ...
        try:
            ...
            # Информация о долгосрочной памяти теперь на сервере
            message += "\n<b>Долгосрочная память:</b> Обрабатывается сервером\n"
            message += "• Семантический поиск: Доступен на сервере\n"
    ...
    def _get_rag_context(self, query: str) -> str:
        """
        Заглушка для метода получения контекста из долгосрочной памяти.
        Вся логика RAG перенесена на сервер.
        
        Args:
            query: Поисковый запрос
            
        Returns:
            str: Пустая строка, так как RAG теперь обрабатывается на сервере
        """
        return ""
