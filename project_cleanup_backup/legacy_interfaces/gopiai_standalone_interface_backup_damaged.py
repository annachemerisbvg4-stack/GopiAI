...
try:
    ...
    )    # Импортируем ThemeManager из GopiAI-Widgets    from gopiai.widgets.managers.theme_manager import ThemeManager
      
    # Создаем простой локальный менеджер иконок    class LucideIconManager:
        _instance = None
...
class FramelessGopiAIStandaloneWindow(QMainWindow):
    ...
    def get_icon(self, icon_name: str, size: QSize = QSize(24, 24)) -> QIcon:
        """Получение иконки через систему LucideIconManager GopiAI"""
        if ICON_MANAGER_AVAILABLE:
            try:
                # Преобразуем имя иконки через маппинг
                lucide_name = get_lucide_name(icon_name)
                print(f"🔍 Маппинг иконки: {icon_name} -> {lucide_name}")
                
                # Используем LucideIconManager для получения иконки
                icon_manager = LucideIconManager.instance()
                icon = icon_manager.get_icon(lucide_name, size=size)
                
                if not icon.isNull():
                    print(f"✓ Иконка {icon_name} загружена через LucideIconManager (размер: {icon.actualSize(size)})")
                    # Проверяем что иконка действительно содержит пиксели
                    pixmap = icon.pixmap(size)
                    print(f"  Pixmap размер: {pixmap.size()}, пустой: {pixmap.isNull()}")
                    return icon
                else:
    ...
    def _setup_menu_icons(self):
        ...
        try:
            ...
            menu_bar = self.titlebar_with_menu.menu_bar
              # Добавляем иконки к существующим действиям меню
            if hasattr(menu_bar, 'new_action'):
                icon = self.get_icon("file-plus", QSize(16, 16))
                menu_bar.new_action.setIcon(icon)
                print(f"✓ Установлена иконка new_action: {not icon.isNull()}")
            if hasattr(menu_bar, 'open_action'):
                icon = self.get_icon("folder-open", QSize(16, 16))
                menu_bar.open_action.setIcon(icon)
                print(f"✓ Установлена иконка open_action: {not icon.isNull()}")
            if hasattr(menu_bar, 'save_action'):
                icon = self.get_icon("save", QSize(16, 16))
                menu_bar.save_action.setIcon(icon)
                print(f"✓ Установлена иконка save_action: {not icon.isNull()}")
            if hasattr(menu_bar, 'productivity_action'):
                icon = self.get_icon("wrench", QSize(16, 16))
                menu_bar.productivity_action.setIcon(icon)
                print(f"✓ Установлена иконка productivity_action: {not icon.isNull()}")
            if hasattr(menu_bar, 'voice_action'):
                icon = self.get_icon("mic", QSize(16, 16))
                menu_bar.voice_action.setIcon(icon)
                print(f"✓ Установлена иконка voice_action: {not icon.isNull()}")
            if hasattr(menu_bar, 'ai_tools_action'):
                icon = self.get_icon("cpu", QSize(16, 16))
                menu_bar.ai_tools_action.setIcon(icon)
                print(f"✓ Установлена иконка ai_tools_action: {not icon.isNull()}")
