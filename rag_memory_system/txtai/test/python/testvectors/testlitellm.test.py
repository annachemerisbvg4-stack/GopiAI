...
class TestLiteLLM(unittest.TestCase):
    ...
    def test_index_multiple(self):
        ...
        ids, dimension, batches, stream = model.index([(0, "test1", None), (1, "test2", None)])
        self.assertEqual(len(ids), 2)
        with open(stream, "rb") as queue:
            self.assertEqual(np.load(queue).shape, (2, 768))

    def test_index_empty(self):
        model = VectorsFactory.create(
            {"path": "huggingface/sentence-transformers/all-MiniLM-L6-v2", "vectors": {"api_base": "http://127.0.0.1:8004"}}, None
        )
        ids, dimension, batches, stream = model.index([])
        self.assertEqual(len(ids), 0)
        self.assertEqual(dimension, 768)
        self.assertEqual(batches, 0)
        self.assertIsNotNone(os.path.exists(stream))
        with open(stream, "rb") as queue:
            self.assertEqual(np.load(queue).shape, (0, 768))
    ...
    def test_config_override(self):
        model = VectorsFactory.create(
            {"path": "huggingface/sentence-transformers/all-MiniLM-L6-v2", "vectors": {"api_base": "http://127.0.0.1:8004", "model_kwargs": {"trust_remote_code": True}}}, None
        )
        ids, dimension, batches, stream = model.index([(0, "test", None)])
