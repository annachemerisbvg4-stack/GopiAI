...
class ExampleWorkflows:
    ...
    async def run_planning_with_parallel_execution(
        ...
        if not tools:
            tools = [WebSearch(), Bash(), StrReplaceEditor()]
    ...
    async def run_agent_rag_workflow(
        query: str, documents: List[Dict[str, Any]], context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        # Create nodes
        generator_node = orchestrator.create_agent_node(
            generator_agent.name, "response"
        )

        # Create a sequential workflow
        workflow = orchestrator.create_sequential_workflow(
            nodes=[retrieval_node, generator_node], flow_name="RAGWorkflow"
        )

        # Run the workflow
        result = orchestrator.run_workflow(
            workflow_name="RAGWorkflow",
            inputs={"query": query, "documents": documents, "context": context or {}},
        )
...
async def main():
    ...
    parser.add_argument("--query", type=str, help="Query for RAG workflow")
    parser.add_argument(
        "--documents",
        type=str,
    ...
    if args.workflow == "multi_agent":
        ...
        result = await ExampleWorkflows.run_agent_rag_workflow(
            query=args.query, documents=documents, context=context
        )
        print("\nWorkflow result:")
        print(json.dumps(result, indent=2))
