...
class LucideIconManager:
    ...
    def __init__(self):
        ...
        try:
            original_path = Path(__file__).parent.parent / "GopiAI-Widgets" / "gopiai" / "widgets" / "managers" / "lucide_icon_manager.py"
            ...
            print(f"❌ Ошибка при инициализации оригинального менеджера: {e}")
        ...
        if self.original_manager is None:
            ...
            print(f"⚠️ Используется локальная версия LucideIconManager")
        else:
