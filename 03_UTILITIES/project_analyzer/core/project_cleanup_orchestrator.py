...
class ProjectCleanupAnalyzer:
    ...
    def _configure_analyzers_by_depth(self):
        ...
        if self.config.analysis_depth == 'quick':
            # Quick analysis - no file limit by default (0 means no limit)
            self.config.max_files_per_analyzer = self.config.max_files_per_analyzer or 0
            self.logger.info("Using 'quick' analysis depth - no file limit")
            
        elif self.config.analysis_depth == 'standard':
            # Standard analysis - no file limit by default (0 means no limit)
            self.config.max_files_per_analyzer = self.config.max_files_per_analyzer or 0
            self.logger.info("Using 'standard' analysis depth - no file limit")
