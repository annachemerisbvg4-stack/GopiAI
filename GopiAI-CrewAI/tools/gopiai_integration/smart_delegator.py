...
class SmartDelegator:
    ...
    def _handle_with_ai_router(self, message: str) -> str:
        ...
        if not self.ai_router:
            return "⚠️ AI Router недоступен. Пожалуйста, проверьте настройки."
        ...
        try:
            ...
            # Отправляем запрос через AI Router
            return self.ai_router.call(enriched_message, temperature=0.7)
        except Exception as e:
            print(f"⚠️ Ошибка при вызове AI Router: {str(e)}")
            # Создаем простой ответ-заглушку
            return f"⚠️ Произошла ошибка при обработке запроса через AI Router: {str(e)}\n\nПожалуйста, проверьте настройки системы или попробуйте позже."
