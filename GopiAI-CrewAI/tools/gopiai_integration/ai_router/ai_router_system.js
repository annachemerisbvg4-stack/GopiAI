...
class AIRouter {
    ...
    /**
     * 🔄 ОБРАБОТКА ЗАПРОСА (совместимость с GopiAI-CrewAI)
     */
    async processRequest(message, taskType = 'general', modelPreference = 'auto') {
        // Этот метод добавлен для обратной совместимости с GopiAI-CrewAI
        console.log(`🔄 Запрос: ${message.substring(0, 50)}... (тип: ${taskType}, модель: ${modelPreference})`);
        ...
        // Если указано предпочтение модели, пытаемся его учесть
        if (modelPreference !== 'auto') {
            // Находим провайдера с указанной моделью
            const preferredProvider = this.providers.find(p => 
                p.model.toLowerCase().includes(modelPreference.toLowerCase()) ||
                p.name.toLowerCase().includes(modelPreference.toLowerCase())
            );
            
            if (preferredProvider) {
                console.log(`🎯 Выбран предпочтительный провайдер: ${preferredProvider.name}`);
                this.currentProvider = preferredProvider;
            }
        }
        
        // Вызываем основной метод chat
