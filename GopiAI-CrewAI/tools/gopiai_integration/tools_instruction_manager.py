...
class ToolsInstructionManager:
    ...
    def get_tools_summary(self) -> Dict[str, str]:
        ...
        # Оставляем только канонические инструменты и добавляем недостающие
        canonical_tools = self.alias_manager.get_canonical_tools()
        # Фильтрация: исключаем неканонические (алиасы) из базового набора
        filtered_tools = {k: v for k, v in base_tools.items() if k in canonical_tools}
        # Добавляем отсутствующие канонические инструменты с базовым описанием
        for tool in canonical_tools:
            if tool not in filtered_tools:
                filtered_tools[tool] = f"Инструмент {tool} (доступен через систему алиасов)"
