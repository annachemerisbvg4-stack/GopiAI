...
class IconFileSystemModel(QFileSystemModel):
    ...
    def _load_svg_icon(self, icon_name: str) -> Optional[QIcon]:
        """Загружает SVG иконку из директории UI/assets/icons/lucide"""
        try:
            # Путь к SVG файлу - поднимаемся на один уровень от UI/components/ к UI/
            ui_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            svg_path = os.path.join(ui_dir, "assets", "icons", "lucide", f"{icon_name}.svg")
