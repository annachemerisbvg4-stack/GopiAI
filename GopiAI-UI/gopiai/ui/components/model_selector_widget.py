...
class ModelSelectorWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        logger.info("🎛️ ModelSelectorWidget инициализирован")
    ...
    def _load_current_configuration(self):
        ...
        try:
            ...
            if current_config:
                ...
                logger.warning("⚠️ Нет активной конфигурации")
    ...
    def _load_models_for_provider(self):
        ...
        try:
            ...
            logger.info(f"📋 Загружено {len(available_models)} моделей для {self.current_provider}")
            ...
            try:
                if self.model_config_manager:
                    ...
                    if success:
                        ...
                        logger.warning(f"⚠️ Не удалось установить модель: {model_id}")
    ...
    def _save_api_key(self, env_var: str, api_key: str):
        ...
        if not api_key.strip():
            logger.warning("⚠️ Пустой API ключ")
            return
            ...
            try:
                ...
                for provider, info in status.items():
                    if info['available_models'] > 0:
                        stats_text.append(f"{provider.upper()}: {info['available_models']} моделей")
        ...
        try:
            ...
            if self.model_config_manager:
                ...
                if success:
                    ...
                    logger.warning("⚠️ Не удалось сбросить конфигурацию")
