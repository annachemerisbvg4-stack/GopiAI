...
try:
    ...
    class SimpleTitlebarWithMenu(QWidget):
        def __init__(self):
            super().__init__()
            layout = QVBoxLayout()
            layout.addWidget(QLabel("Fallback: TitlebarWithMenu"))
            self.setLayout(layout)
    ...
    StandaloneTitlebar = lambda: SimpleWidget("Titlebar")
    StandaloneTitlebarWithMenu = SimpleTitlebarWithMenu
    CustomGrip = lambda parent, direction: QWidget()
    FileExplorerWidget = lambda: SimpleWidget("FileExplorer")
    TabDocumentWidget = lambda: SimpleWidget("TabDocument")
    ChatWidget = lambda: SimpleWidget("Chat")
    TerminalWidget = lambda: SimpleWidget("Terminal")
    ThemeManager = None
    AutoIconSystem = None

# Импорт системы тем (fallback)
try:
...
class FramelessGopiAIStandaloneWindow(QMainWindow):
    ...
    def _init_grips(self):
        ...
        if not MODULES_LOADED:
            print("⚠️ Грипы недоступны в fallback режиме")
            return
    ...
    def _connect_menu_signals(self):
        ...
        if not MODULES_LOADED:
            print("⚠️ Сигналы меню недоступны в fallback режиме")
            return
    ...
    def _on_save_file(self):
        """Сохранение файла"""
        if MODULES_LOADED and hasattr(self.tab_document, 'get_current_text'):
            ...
            print("💾 Сохранение файла (fallback режим)")
