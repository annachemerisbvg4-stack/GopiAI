...
from PySide6.QtCore import Qt as QtCore  # Импортируем как QtCore для явного использования
from PySide6.QtGui import QIcon, QPixmap, QPainter, QColor, QPen, QFont, QFont
from PySide6.QtGui import QPainter as QPainterClass  # Для доступа к константам
...
class UniversalIconManager:
    ...
    def _init_lucide_manager(self):
        ...
        for import_path in [
            "gopiai.widgets.managers.lucide_icon_manager",
            "ui_components.lucide_icon_manager",
            ".lucide_icon_manager",
        ]:
            try:
                if import_path == ".lucide_icon_manager":
                    # Локальный импорт
                    try:
                        from .lucide_icon_manager import LucideIconManager
                        self.lucide_manager = LucideIconManager()
                        
                        # Проверяем функциональность
                        if hasattr(self.lucide_manager, "get_icon"):
                            logger.info(f"Загружен локальный LucideIconManager")
                            return
                        else:
                            logger.warning("Локальный LucideIconManager не имеет метода get_icon")
                            self.lucide_manager = None
                    except ImportError as e:
                        logger.debug(f"Не удалось импортировать локальный LucideIconManager: {e}")
                        continue
                else:
                    # Импорт из модуля
                    module = __import__(import_path, fromlist=["LucideIconManager"])
                    LucideIconManager = getattr(module, "LucideIconManager")
                    
                    # Если это синглтон
                    if hasattr(LucideIconManager, "instance"):
                    ...
                    # Проверяем функциональность
                    if hasattr(self.lucide_manager, "get_icon"):
                        logger.info(f"Загружен LucideIconManager из {import_path}")
                        return
                    else:
                        logger.warning(f"LucideIconManager из {import_path} не имеет метода get_icon")
                        self.lucide_manager = None
            except ImportError:
                logger.debug(f"Не удалось импортировать LucideIconManager из {import_path}")
            except Exception as e:
                logger.error(f"Ошибка при инициализации LucideIconManager из {import_path}: {e}")
        
        # Если все импорты провалились
        logger.warning("Не удалось загрузить ни один LucideIconManager")
        self.lucide_manager = None
